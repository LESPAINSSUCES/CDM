 ...
 * Source live : worldcup26.ir via proxy Supabase + lien FIFA + resultats.json (organisateur).
 */
(function (global) {
  ...
  function normTeam(s) {
    return global.CDM_WC26?.normTeam(s) ?? String(s || '').toLowerCase().trim();
  }

  function teamLabel(name) {
    return global.CDM_WC26?.teamLabel(name) ?? name;
  }
  ...
  async function fetchFixtures() {
    const wc = global.CDM_WC26;
    if (!wc?.fetchLiveFixtures) {
      throw new Error('Module worldcup26-api.js manquant — rechargez la page.');
    }

    const [livePayload, resultats] = await Promise.all([
      wc.fetchLiveFixtures(),
      fetchResultats(),
    ]);

    const items = (livePayload.fixtures || []).map((fx) => ({
      raw: fx.raw,
      home: fx.home,
      away: fx.away,
      group: fx.group,
    ...
    return {
      ...
      resultats,
      proxyWarning: livePayload.error || null,
    };
  }

  function renderWidget(root, payload) {
    const { items, fetchedAt, resultats, source, proxyWarning } = payload;
    ...
    const warn = proxyWarning
      ? `<p class="live-scores-warn">${escapeHtml(String(proxyWarning))}</p>`
      : '';
    ...
    root.innerHTML = `
      <div class="live-scores-head">
        <div class="live-scores-head-main">
          ...
          <span class="live-scores-updated">Màj ${escapeHtml(updated)} · ${escapeHtml(source || 'worldcup26')}</span>
        </div>
  ...
  function renderError(root, err) {
    const msg = escapeHtml(err?.message || 'Erreur inconnue');
    root.className = 'live-scores-widget live-scores-error';
    root.innerHTML = `
      <div class="live-scores-head">
        <div class="live-scores-head-main">
          <h2 class="live-scores-title">Scores live</h2>
          <span class="live-scores-updated">Indisponible</span>
        </div>
        <a class="live-scores-fifa-link" href="${FIFA_SCORES_URL}" target="_blank" rel="noopener">FIFA →</a>
      </div>
      <p class="live-scores-msg live-scores-err">${msg}</p>
      <p class="live-scores-hint">Le classement du concours utilise <strong>resultats.json</strong> (organisateur), pas ce widget.</p>
      <button type="button" class="live-scores-retry">Réessayer</button>`;
    const btn = root.querySelector('.live-scores-retry');
    if (btn) btn.addEventListener('click', () => refresh(root));
  }
